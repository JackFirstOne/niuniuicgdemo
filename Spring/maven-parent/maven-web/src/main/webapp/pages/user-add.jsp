			...
			<form action="${pageContext.request.contextPath}/user/addUser"
				...
				<div class="panel panel-default">
					...
					<div class="row data-type">
						...
						<div class="col-md-2 title" style="display: none;">用户id</div>
						...
						<div class="col-md-10 data">
							...
							<c:forEach var="role" items="${roleList}">
								<c:choose>
									<c:when test="${user.roles!=null&&user.roles.size()>0}">
										<input class="" id="${role.id}" type="checkbox" name="roleId" value="${role.id}">${role.roleName}
									    <%--<c:forEach  var="userRole" items="${user.roles}">
											<c:if test="${roleList.contains(userRole)}">
												<script type="text/javascript">
													var userRole = "${userRole}";
													$("#"+userRole).setAttribute("checked",true);
												</script>
											</c:if>
										</c:forEach>--%>
									</c:when>
